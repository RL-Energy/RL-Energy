...
def RL_run(env,RL,GNN,user_inputs,all_available,f_rec,P):
	...
	repeated_obs_runIdaes_flowsheet = []
	repeated_obs_runIdaes_reward = []
	repeated_obs_runIdaes_extra_reward = []
	repeated_obs_runIdaes_status = []
	repeated_obs_runIdaes_costing = []
	i_idaes_repeated = 0
	...
	ind_col_spe = temp1_col.index('flash_0.liq_outlet')
	...
	for episode in range(Episode_max):
			...
			if GNN_class == True and pass_pre_screen == True and episode_done == True:
				...
				if IDAES_enable == True:
					...
					if list_flowsheet in obs_runIdaes_flowsheet:
						...
						i_idaes_repeated += 1
						repeated_obs_runIdaes_flowsheet.append(list_flowsheet)
						repeated_obs_runIdaes_reward.append(reward_)
						repeated_obs_runIdaes_extra_reward.append(extra_reward_)
						repeated_obs_runIdaes_status.append(IDAES_status)
						repeated_obs_runIdaes_costing.append(IDAES_costing)
					else:
						i_idaes_unique += 1
		...
		if episode % 1000 == 0 and i_idaes_unique>0: 
			...
			if i_idaes_repeated>0:
				tmp_array = np.array(repeated_obs_runIdaes_flowsheet)
				fmt = ",".join(["%d"] * len(tmp_array[0]))
				np.savetxt(dir_result+'repeated_obs_runIdaes_flowsheet_'+str(model_index)+'.csv',np.vstack(tmp_array),fmt=fmt,comments='')
				tmp_array = np.array(repeated_obs_runIdaes_reward)
				fmt = ",".join(["%d"])
				np.savetxt(dir_result+'repeated_obs_runIdaes_reward_'+str(model_index)+'.csv',tmp_array,fmt=fmt,comments='')
				tmp_array = np.array(repeated_obs_runIdaes_extra_reward)
				fmt = ",".join(["%d"])
				np.savetxt(dir_result+'repeated_obs_runIdaes_extra_reward_'+str(model_index)+'.csv',tmp_array,fmt=fmt,comments='')
				tmp_array = np.array(repeated_obs_runIdaes_status)
				tmp_array = np.array(tmp_array[:,1:], dtype=float)
				fmt = ",".join(["%f"]*len(tmp_array[0]))
				np.savetxt(dir_result+'repeated_obs_runIdaes_status_'+str(model_index)+'.csv',np.vstack(tmp_array),fmt=fmt,comments='')
				tmp_array = np.array(repeated_obs_runIdaes_costing)
				fmt = ",".join(["%f"]*len(tmp_array[0]))
				np.savetxt(dir_result+'repeated_obs_runIdaes_costing_'+str(model_index)+'.csv',np.vstack(tmp_array),fmt=fmt,comments='')
		...
		done_reward_hist.append(reward_+extra_reward_)
